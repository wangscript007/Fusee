...
namespace Fusee.Engine.Imp.Graphics.WebAsm
{
    ...
    public class KeyboardDeviceImp : IInputDeviceImp
    {
        ...
        private readonly FusCanvas _canvas;

        #region JS Connectors

        private static EventHandler<KeyboardEventArgs> OnKeyDownEvent;
        private static EventHandler<KeyboardEventArgs> OnKeyUpEvent;

        [JSInvokable]
        public static void OnKeyDown(KeyboardEventArgs e)
        {
            OnKeyDownEvent?.Invoke(null, e);
        }

        [JSInvokable]
        public static void OnKeyUp(KeyboardEventArgs e)
        {
            OnKeyUpEvent?.Invoke(null, e);
        }        
        ...
        private void ConnectCanvasEvents()
        {
            OnKeyDownEvent += (_, e) =>
            {
                var ev = (KeyboardEventArgs)e;
                OnCanvasKeyDown(int.Parse(ev.Code));
            };

            OnKeyUpEvent += (_, e) =>
            {
                var ev = (KeyboardEventArgs)e;
                OnCanvasKeyUp(int.Parse(ev.Code));
            };
        ...
        internal KeyboardDeviceImp(FusCanvas canvas)
        {
    ...
    public class MouseDeviceImp : IInputDeviceImp
    {
        ...
        private static EventHandler<Microsoft.AspNetCore.Components.Web.MouseEventArgs> OnMouseDownEvent;
        private static EventHandler<Microsoft.AspNetCore.Components.Web.MouseEventArgs> OnMouseUpEvent;
        private static EventHandler<Microsoft.AspNetCore.Components.Web.MouseEventArgs> OnMouseMoveEvent;
        private static EventHandler<Microsoft.AspNetCore.Components.Web.WheelEventArgs> OnMouseWheelEvent;

        [JSInvokable]
        public static void OnMouseDown(Microsoft.AspNetCore.Components.Web.MouseEventArgs e)
        {
            OnMouseDownEvent?.Invoke(null, e);
        }

        [JSInvokable]
        public static void OnMouseUp(Microsoft.AspNetCore.Components.Web.MouseEventArgs e)
        {
            OnMouseUpEvent?.Invoke(null, e);
        }

        [JSInvokable]
        public static void OnMouseMove(Microsoft.AspNetCore.Components.Web.MouseEventArgs e)
        {
            OnMouseMoveEvent?.Invoke(null, e);
        }

        [JSInvokable]
        public static void OnMouseWheel(WheelEventArgs e)
        {
        ...
        private void ConnectCanvasEvents()
        {
            ...
            OnMouseDownEvent += (_, e) =>
            {
                OnCanvasMouseDown((int)e.Buttons);
            };

            OnMouseUpEvent += (_, e) =>
            {
                OnCanvasMouseUp((int)e.Buttons);
            };

            OnMouseMoveEvent += (_, e) =>
            {
                // TODO (mr): Check if we need the offset or the absolute position
                var currentMousePos = new float2((float)e.ClientX, (float)e.ClientY);
                var offset = currentMousePos - _lastMousePosition;
                OnCanvasMouseMove(offset);

                _lastMousePosition = currentMousePos;
            };

            OnMouseWheelEvent += (_, e) =>
            {
                OnCanvasMouseWheel((float)e.DeltaY);
            };
        ...
        private float GetWindowWidth => _canvas.Width;


        private float GetWindowHeight => _canvas.Height;
        ...
        public MouseDeviceImp(FusCanvas canvas)
        {
    ...
    public class TouchDeviceImp : IInputDeviceImp
    {
        ...
        private static EventHandler<TouchEventArgs> OnTouchStartEvent;
        private static EventHandler<TouchEventArgs> OnTouchEndEvent;
        private static EventHandler<TouchEventArgs> OnTouchCancelEvent;
        private static EventHandler<TouchEventArgs> OnTouchMoveEvent;


        [JSInvokable]
        public static void OnTouchStart(TouchEventArgs e)
        {
            OnTouchStartEvent?.Invoke(null, e);
        }

        [JSInvokable]
        public static void OnTouchEnd(TouchEventArgs e)
        {
            OnTouchEndEvent?.Invoke(null, e);
        }

        [JSInvokable]
        public static void OnTouchCancel(TouchEventArgs e)
        {
            OnTouchCancelEvent?.Invoke(null, e);
        }

        [JSInvokable]
        public static void OnTouchMove(TouchEventArgs e)
        {
            OnTouchMoveEvent?.Invoke(null, e);
        }

        private void ConnectCanvasEvents()
        {

            OnTouchStartEvent += (_, e) =>
            {
                for(var i = 0; i < e.Touches.Length; i++)
                {
                    var touch = e.Touches[i];
                    OnCanvasTouchStart((int)touch.Identifier, (float)touch.PageX, (float)touch.PageY);
                }
            };
            ...
            OnTouchEndEvent += (_, e) =>
            {
                ...
                {
                    var touch = e.Touches[i];
                    OnCanvasTouchEnd((int)touch.Identifier, (float)touch.PageX, (float)touch.PageY);
                }
            };


            OnTouchCancelEvent += (_, e) =>
            {
                for (var i = 0; i < e.Touches.Length; i++)
                {
                    var touch = e.Touches[i];
                    OnCanvasTouchCancel((int)touch.Identifier, (float)touch.PageX, (float)touch.PageY);
                }
            };

            OnTouchMoveEvent += (_, e) =>
            {
                for (var i = 0; i < e.Touches.Length; i++)
                {
                    var touch = e.Touches[i];
                    OnCanvasTouchMove((int)touch.Identifier, (float)touch.PageX, (float)touch.PageY);
                }
            };
        }
        ...
        public TouchDeviceImp(FusCanvas canvas)
        {
            _canvas = canvas;
            ConnectCanvasEvents();
