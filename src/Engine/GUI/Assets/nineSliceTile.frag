...
void main()
{
	...
	{		
		...
		if(currentTileX == 0.0)
		{
			...
			outColor = vec4(texture(AlbedoTexture, vec2(uvX , uvY)) * AlbedoMix) * AlbedoColor *  max(dot(N, L), 0.0);
		}
		//Last tile
		...
		outColor = vec4(texture(AlbedoTexture,vUV) * AlbedoMix)* AlbedoColor *  max(dot(N, L), 0.0);	
}